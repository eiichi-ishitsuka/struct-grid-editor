...
describe('Phase 3: 行・列操作 (Row & Column Operations)', function () {
     ...
     */
    it.skip('UC-09: テーブル行追加ボタン押下で行数が増加すること', async () => {
        if (!page) {
            return;
        }
     ...
     */
    it.skip('UC-10: 行を選択して削除した際に行数が減少すること', async () => {
        if (!page) {
            return;
        }
     ...
     */
    it.skip('UC-11: テーブル列追加ボタン押下で新しい列が追加されること', async () => {
        if (!page) {
            return;
        }
     ...
     */
    it.skip('UC-12: 「テキストで開く」ボタン押下でテキストエディタに切り替わること', async () => {
        if (!page) {
            return;
        }
